    ...
    public class clsRangeQuery
    {
        ...
        public void RangeQuery(string strUrl, ESRI.ArcGIS.Client.Geometry.Geometry geotmp, List<Graphic> lsttmp)
        {
            ...
            if (PFApp.MapServerType == enumMapServerType.Baidu)
                RangeQueryByToolKit(geotmp);
            else if (PFApp.MapServerType == enumMapServerType.Esri)
            {
                RangeQueryByToolKit(geotmp);
                //RangeQueryByService(strUrl, geotmp);
            }
            else if (strUrl != "")
            {
                RangeQueryByToolKit(geotmp);
        ...
        void RangeQueryByToolKit(ESRI.ArcGIS.Client.Geometry.Geometry geotmp)
        {
            foreach (var item in lstGra)
            {
                if (ToolKitForNonArcGISLayer.CheckRegion(geotmp as Polygon, (MapPoint)item.Geometry))
                {
                    lstReturnGraphic.Add(item);
                }
            }
            ProcessAction(this, EventArgs.Empty);
        }

        void RangeQueryByService(string strUrl,ESRI.ArcGIS.Client.Geometry.Geometry geotmp)
        {
            geometryservice = new GeometryService(strUrl);
            geometryservice.Failed -= geometryservice_Failed;
            geometryservice.Failed += new EventHandler<TaskFailedEventArgs>(geometryservice_Failed);
