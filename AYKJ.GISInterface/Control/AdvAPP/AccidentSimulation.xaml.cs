...
{
    public partial class AccidentSimulation : UserControl
    {
        //定义一个Map用来接收平台的Map
        Map mainmap;
        //绘制的Draw
        ...
        public void CallDrawAccidentSimulation(string modetype, List<object> lst)
        {
            //设置面板的起始位置
        ...
        /// <param name="lst">事故参数</param>
        public void CallNameAccidentSimulation(string modetype, string strname, List<object> lst)
        {
            //设置面板的起始位置
            this.VerticalAlignment = System.Windows.VerticalAlignment.Top;
            this.HorizontalAlignment = System.Windows.HorizontalAlignment.Right;
            this.Margin = new Thickness() { Top = 60, Right = 13 };
            ...
            if (PFApp.Root.Children.Count > 0)
            {
                ...
                {
                    if (PFApp.Root.Children[i].GetType().Name == "AccidentSimulation")
                    {
        ...
        void CallLeakModel(double dbx, double dby, string strtype)
        {
            switch (strtype)
            {
                ...
                case "连续泄漏":
                    ...
                    {
                        CallContinuousLeak(ary4, ary5, dbx, dby, dbCombustionStep, dbPoisoningStep, dbCombustionStart,
                                                  dbPoisoningStart, strcontinuetype, Double.Parse(atyobj[7].ToString()));
                    }
                    break;
            }
        }
        ...
        public void CallContinuousLeak(double[] aryCombustion, double[] aryPoisoning, double dbx, double dby, double dbCombustionStep,
            double dbPoisoningStep, double dbCombustionStart, double dbPoisoningStart, string strtype, double dbrangle)
        {
            ...
            for (int i = 0; i < value + 1; i++)
            {
                ...
                for (int m = 0; m < tmpdata.Count; m++)
                {
                    ...
                    if (!Dict_data.Keys.ToList().Contains(tmpdata[m].wxyid))
                        Dict_data.Add(tmpdata[m].wxyid,
                            new clstipwxy()
                            {
                                wxydwdm = tmpdata[m].wxydwdm,
                                wxyid = tmpdata[m].wxyid,
                                wxyname = tmpdata[m].wxyname,
                                wxytype = tmpdata[m].wxytype,
                                wxytip = tmpdata[m].wxytip
                            });
                }
            }
            data_result.ItemsSource = null;
