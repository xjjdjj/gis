...
namespace AYKJ.GISInterface
{
    ...
    /// <param name="e"></param>
    public delegate void ContinueRouteAnalysisDelegate(object sender, EventArgs e);

    public class clsContinueRouteAnalysis
    {
        //定义事件
        public event ContinueRouteAnalysisDelegate RouteAnalysisEvent;
        public event ContinueRouteAnalysisDelegate RouteAnalysisFaildEvent;
        //救援点集合
        ...
        //最短事故区域内逃生路径类
        clsContinueSafePoint clsContinuesafepoint;
        //泄漏区域
        Graphic LeakArea;
        //计算次数
        int count;
        ...
        void ProcessAction(object sender, EventArgs e)
        {
            if (RouteAnalysisEvent == null)
                RouteAnalysisEvent += new ContinueRouteAnalysisDelegate(RouteAnalysisErrorEvent);
        ...
        public void Route(List<Graphic> lstrescue, List<Graphic> lstrecipient, List<Graphic> lstsafety, MapPoint mpaccident, Graphic leakarea, Map mp, string strgurl, string strnurl, string strcfurl, string strqurl)
        {
        ...
        void clsrescueroute_RescueRouteEvent(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    clsContinuesafepoint = new clsContinueSafePoint();
                    clsContinuesafepoint.SafePointEvent += new ContinueSafePointDelegate(clssafepoint_SafePointEvent);
                    clsContinuesafepoint.SafePointFaildEvent += new ContinueSafePointDelegate(clssafepoint_SafePointFaildEvent);

                    clsContinuesafepoint.SafeRoute(map, lst_Recipient[count].Geometry as MapPoint, Mp_Accident, LeakArea, strgeourl, strclosestfacilityurl, strqueryurl);
                }
                else
                {
        ...
        {
            List<Graphic> lst = (sender as clsContinueSafePoint).Return_lst;
