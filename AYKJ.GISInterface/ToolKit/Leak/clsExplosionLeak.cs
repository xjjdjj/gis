...
namespace AYKJ.GISInterface
{
    ...
    /// <param name="e"></param>
    public delegate void ExplosionLeakDelegate(object sender, EventArgs e);

    public class clsExplosionLeak
    {
        #region 同心圆的参数
        //泄漏分析的Map
        Map LeakMap;
        ...
        #endregion
        //定义查询事件
        public event ExplosionLeakDelegate ExplosionLeakEvent;
        public event ExplosionLeakDelegate ExplosionLeakFaildEvent;
        #region 返回参数
        //返回圆心列表
        ...
        {
            if (ExplosionLeakEvent == null)
                ExplosionLeakEvent += new ExplosionLeakDelegate(ExplosionLeaErrorEvent);
            ExplosionLeakEvent(sender, e);
        }
        ...
        {
            ExplosionLeakFaildEvent(sender, e);
        }
