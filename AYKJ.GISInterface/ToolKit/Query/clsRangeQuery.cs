...
{
    public class RangeQueryEventArgs : EventArgs
    {
    ...
    /// <param name="e"></param>
    public delegate void RangeQueryDelegate(object sender, RangeQueryEventArgs e);
    ...
    public class clsRangeQuery
    {
        ...
        void ProcessAction(object sender, RangeQueryEventArgs e)
        {
        ...
        {
            RangeQueryEventArgs ev = new RangeQueryEventArgs();
            ev.Message = e.ToString();
            RangeQueryFaildEvent(sender, ev);
        }
        ...
        public void RangeQuery(ESRI.ArcGIS.Client.Geometry.Geometry geotmp, List<Graphic> lsttmp)
        {
            //客户端进行相交匹配
            lstReturnGraphic = new List<Graphic>();
            lstGra = lsttmp;
            ...
            {
                Graphic g = lsttmp[i];
                if (ToolKitForNonArcGISLayer.CheckRegion(geotmp as Polygon, g.Geometry as MapPoint))
                {
                    lstReturnGraphic.Add(g);
                }
            }
            foreach (Graphic g in lstReturnGraphic)
            {
                //lsttmp.Remove(g);
                lstGra.Remove(g);
            }
            RangeQueryEventArgs ev = new RangeQueryEventArgs();
            ev.Message = "ok";
            ProcessAction(this, ev);
        }

        public void MutilRangeQuery(List<ESRI.ArcGIS.Client.Geometry.Polygon> list_polygon, MapPoint point)
        {
            centerPoint = point;
            //客户端进行相交匹配
            lstReturnGraphic = new List<Graphic>();
            ReturnIndex = ToolKitForNonArcGISLayer.CheckMutilRegion(list_polygon, point);
        ...
        public void RangeQuery(string strUrl, ESRI.ArcGIS.Client.Geometry.Geometry geotmp, List<Graphic> lsttmp)
        {
            ...
            if (geometryservice.IsBusy == false)
            {
                ...
                {
                    RangeQueryEventArgs ev = new RangeQueryEventArgs();
                    ev.Message = "exception:" + e.Message;
                    ProcessAction(this, ev);
                }
            }
            else
            {
                RangeQueryEventArgs ev = new RangeQueryEventArgs();
                ev.Message = "geometryservice is busy.";
                ProcessAction(this, ev);
            }
        }
