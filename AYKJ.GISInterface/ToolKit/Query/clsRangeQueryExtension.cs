...
namespace AYKJ.GISInterface
{
    ...
    public class clsRangeQueryExtension
    {
        ...
        public void QueryExtension(string str, List<Graphic> lst)
        {
            ...
            Graphic[] tmpGras = new Graphic[(Application.Current as IApp).lstThematic.Count];
            (Application.Current as IApp).lstThematic.CopyTo(tmpGras);
            lstbygra = new List<Graphic>();
            foreach (Graphic gra in tmpGras)
                lstbygra.Add(gra);
        ...
        void QueryExtensionByToolKit()
        {
            if (lstgra[int_count] == null)
            {
                int_count = 1;
                if (lstgra[1] == null)
                {
                    ...
                    {
                        int_count = 3;
                        clsrangequery.RangeQuery( lstgra[int_count].Geometry, lstbygra);
                    }
                    else
                    {
                        clsrangequery.RangeQuery( lstgra[int_count].Geometry, lstbygra);
